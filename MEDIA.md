...
## 2-1. 작업 폴더 (`바탕화면\주영 작업`) 쓰는 법

프로젝트마다 폴더가 하나씩 있고, 이름 뒤에 현재 개수가 붙습니다.

```
파타야 [6, 0, 0, 11]
       └ 이미지  └ 페이지 완료  └ 유튜브만  └ 미업로드
```
...
| 폴더 | 내용 |
|---|---|
| `사진 업로드` | 사이트에 올라간 사진 (줄인 것) |
| `사진 미업로드` | 아직 안 올린 사진 |
| `사진 원본` | 원본 고화질 |
| `영상 미업로드` | 아직 유튜브에 안 올린 영상 |
| `유튜브 업로드 완료` | 유튜브에는 올렸지만 페이지엔 아직 |
| `페이지 업로드 완료` | 페이지에 링크까지 들어간 영상 |

### 새 자료를 넣을 때

**프로젝트 폴더 최상위에 그냥 던져놓고** 아래를 실행하면 됩니다.

```bash
powershell -ExecutionPolicy Bypass -File tools/sync-folder-names.ps1
```

- 사진은 줄여서 `사진 업로드`로, 원본은 `사진 원본`으로
- 영상은 `영상 미업로드`로
- 이름을 `project_<slug>_<번호>`로 바꾸고
- 사이트 폴더에 반영하고
- 폴더 이름의 숫자를 갱신합니다

**이미 번호가 붙은 파일은 건드리지 않습니다.** 유튜브 링크와 어긋나면 안 되기 때문입니다.

### 영상을 유튜브에 올린 뒤

주소만 알려주시면 됩니다. 유튜브 제목으로 어느 파일인지 찾아 링크를 넣고,
스크립트가 데이터 파일의 주석을 읽어 그 파일을 `페이지 업로드 완료`로 옮깁니다.

```ts
youtube: [
  // project_jeju-tablet-app_1.mp4   ← 이 주석이 파일 이동의 기준입니다
  { url: "https://youtu.be/wREVoiT9vx8", label: "스토리모드 디지털북" },
],
```

> `건드리지마시오`, `키키키런` 폴더는 스크립트가 건너뜁니다.
